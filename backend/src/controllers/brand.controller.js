...
export async function getAdminBrand(req,res){const item=await prisma.brand.findUnique({where:{id:req.params.id},include:{_count:{select:{products:true}}}});if(!item)throw new AppError(404,"BRAND_NOT_FOUND","Marque introuvable.");ok(res,item);}
async function uniqueSlug(base){let value=base,index=2;while(await prisma.brand.findUnique({where:{slug:value},select:{id:true}}))value=`${base}-${index++}`;return value;}
export async function createBrand(req,res){ok(res,await prisma.brand.create({data:{...req.body,slug:await uniqueSlug(req.body.slug)}}),201);}
