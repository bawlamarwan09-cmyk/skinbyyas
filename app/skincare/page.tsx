import type { Metadata } from "next";
import { CommercePage } from "../components/CommercePage";
import { CatalogProducts } from "../components/StoreData";
export const metadata: Metadata = { title: "Skincare à Agadir | Produits Skincare | Skin by Yas", description: "Découvrez les produits skincare disponibles chez Skin by Yas à Agadir et trouvez les soins adaptés à votre routine." };
export default function Page(){ return <CommercePage eyebrow="SKIN BY YAS — AGADIR" title="Produits Skincare à Agadir" intro="Découvrez la sélection skincare de Skin by Yas. Explorez nos produits disponibles en ligne et composez votre routine selon vos besoins et vos préférences."><CatalogProducts category="skincare"/></CommercePage>; }
